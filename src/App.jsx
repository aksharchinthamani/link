...
// ── Gesture roles ───────────────────────────────────────────────────────────
// These gestures have special buffer roles rather than adding to the sentence.
const SPEAK_GESTURE  = 'Yes / Agree';      // ✊ Fist  → speak sentence
const CLEAR_GESTURE  = 'Call Me';           // 🤙      → clear buffer
const SKIP_GESTURES  = new Set([
  'Analyzing...',
  SPEAK_GESTURE,
  CLEAR_GESTURE,
]);
...
function App() {
  ...
  const lastAddedWordRef = useRef(null); // debounce identical consecutive signs
  ...
  // ── Speak the whole sentence buffer ──────────────────────────────────────
  ...
  const handleGestureDetected = useCallback((gesture) => {
    ...
    // Prevent adding the same word twice in a row (debounce)
    if (gesture === lastAddedWordRef.current) return;
    lastAddedWordRef.current = gesture;

    setSentenceBuffer(prev => [...prev, gesture]);

    // Reset debounce after 2 s so the same word CAN be added again later
    setTimeout(() => {
      lastAddedWordRef.current = null;
    }, 2000);
  }, [speakSentence, clearBuffer]);
    ...
    <div className="relative min-h-screen bg-[#020617] text-white overflow-x-hidden">
      ...
      <div className="relative z-10 p-4 pb-6">
        ...
        <div className="max-w-6xl mx-auto mb-4 flex flex-wrap justify-center gap-3">
          {[
            { emoji: '👋', label: 'Hi/Hello → adds to buffer' },
            { emoji: '✊', label: 'Fist → Speak sentence' },
            { emoji: '🤙', label: 'Call Me → Clear buffer' },
          ].map(({ emoji, label }) => (
        ...
        >
          ...
          {/* ── USER A'S SIDE (Sign → Buffer → Speech) ─────────────────── */}
          <div className={`flex flex-col space-y-3 transition-all duration-500 ${focusMode ? 'h-full w-full' : ''}`}>
            ...
            >
              ...
              {/* Buffer word count pill — shows when buffer has words */}
              {sentenceBuffer.length > 0 && (
                <div className="absolute top-14 left-4 glass-emerald px-3 py-1 rounded-full z-10 animate-[fadeInScale_0.25s_ease-out]">
                  <span className="text-xs text-emerald-300 font-semibold">
                    {sentenceBuffer.length} word{sentenceBuffer.length !== 1 ? 's' : ''} buffered
                  </span>
                </div>
              )}

              {/* Latest signed word pop */}
              {sentenceBuffer.length > 0 && (
                <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
                  <span
                    key={sentenceBuffer[sentenceBuffer.length - 1] + sentenceBuffer.length}
                    className="inline-block px-4 py-1.5 rounded-full text-base font-bold text-emerald-200
                               bg-emerald-500/20 border border-emerald-400/40
                               shadow-[0_0_20px_rgba(16,185,129,0.4)]
                               animate-[fadeInScale_0.3s_ease-out]"
                  >
                    +&nbsp;{sentenceBuffer[sentenceBuffer.length - 1]}
                  </span>
                </div>
              )}
                ...
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-semibold text-white drop-shadow">
                    ...
                    <span className="text-emerald-300 font-bold">
                      {sentenceBuffer.length > 0 ? sentenceBuffer[sentenceBuffer.length - 1] : 'Waiting…'}
                    </span>
                  </span>
                  {isTtsSpeaking && (
        ...
        {/* ── Sentence Builder Buffer Bar ─────────────────────────────────── */}
        <SentenceBuffer
          words={sentenceBuffer}
        ...
        <DeveloperConsole fps={fps} landmarks={landmarks} />
      </div>
    </div>
  );
}
