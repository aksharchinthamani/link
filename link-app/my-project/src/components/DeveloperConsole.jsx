...
export default function DeveloperConsole({ fps, landmarks, modelState = 'Idle' }) {
  const [isOpen, setIsOpen] = useState(false);

  const stateColor =
    modelState === 'Running' ? 'text-emerald-400 bg-emerald-400/10' :
    modelState === 'Loading' ? 'text-yellow-400 bg-yellow-400/10' :
    modelState === 'Error'   ? 'text-red-400 bg-red-400/10' :
        ...
        <div className="bg-slate-900/95 border border-slate-700 rounded-lg p-4 mb-2 shadow-2xl backdrop-blur-md w-64 text-sm text-slate-300 font-mono">
          ...
          <div className="space-y-2 mt-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`font-semibold px-2 py-0.5 rounded ${stateColor}`}>{modelState}</span>
            </div>
          </div>
        </div>
      )}
