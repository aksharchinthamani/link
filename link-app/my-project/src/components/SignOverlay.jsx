...
export default function SignOverlay({ webcamRef, onMetricsUpdate, onGestureDetected, onModelStateChange }) {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        if (onModelStateChange) onModelStateChange('Loading');
        landmarkerRef.current = await createHandLandmarker();
        if (onModelStateChange) onModelStateChange('Running');
        animate();
      } catch (err) {
        console.error("Error initializing MediaPipe:", err);
        if (onModelStateChange) onModelStateChange('Error');
      }
    };
    ...
    const animate = () => {
      ...
      if (lastTimeRef.current === 0) lastTimeRef.current = startTimeMs;
        ...
        try {
          ...
          if (results.landmarks && results.landmarks.length > 0) {
            ...
            if (gesture === consecutiveSignRef.current) {
              ...
              // Fire callback exactly once when the gesture is held for 20 frames
              ...
              // New gesture — reset the stability counter
  ...
  }, [webcamRef, onMetricsUpdate, onGestureDetected, onModelStateChange]);

  return (
    // NOTE: No scaleX(-1) here — the webcam feed already uses mirrored={true},
    // so the canvas coordinates match the mirrored display without extra flipping.
    <canvas
      ref={canvasRef}
